...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Elin kapladığı alanın minimum ve maksimum koordinatlarını hesapla
            x_min = min([coord[0] for coord in landmarks])
            y_min = min([coord[1] for coord in landmarks])
            x_max = max([coord[0] for coord in landmarks])
            y_max = max([coord[1] for coord in landmarks])
